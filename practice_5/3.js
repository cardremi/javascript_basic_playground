...
function multiplyTheOdds(arrOperand1, arrOperand2) {
    ...
    const res = [];
    const numOfLoops = arrOperand1.length > arrOperand2.length ? arrOperand1.length : arrOperand2.length; 
    for (let i = 0; i < numOfLoops; i++) {
        if(!arrOperand1[i] || !arrOperand2[i]){
            res.push(0)
        }else{
            res.push(arrOperand1[i] * arrOperand2[i])
        }
    }
    return multiply(res);
}
...
function multiply(arr){
    ...
    for (let i = 0; i < arr.length; i++) {
        if(arr[i] % 2 !== 0){
            sum = sum != undefined ? sum*arr[i] : arr[i];
        }
    }
    return sum;
}
